...
{	 
    ...
    q_home.z = -1.5708f;   // 小臂朝向白色齿轮(这里的弧度是相对于大臂)
...
void ARM2(void *pvParameters)
{
    ...
    Emm_Init(Joint[0].zdt, 0x01, &huart9);
    Emm_Init(Joint[1].zdt, 0x02, &huart5);
    Emm_Init(Joint[2].zdt, 0x03, &huart8);

    vTaskDelay(1500);
	
    // xSemaphoreTake(action_semaphore, 20);
    ...
    Joint[2].zdt_st.exp_pos = DEG_TO_RAD(start_rad[2]);
		...
		{
			Emm_V5_Position(Joint[i].zdt, Joint[i].zdt_st.exp_pos > 0 ? DIR_CW : DIR_CCW, 20, 250, Emm_Angle_To_Pulses(RAD_TO_DEG(fabsf(Joint[i].zdt_st.exp_pos * Joint[i].dir))), true, false);
		}
		...
		for (int i = 0; i < 3; i++) 
	 {
		  ...
		  vTaskDelay(1000);
			...
			Emm_V5_Position(Joint[i].zdt, Joint[i].zdt_st.exp_pos > 0 ? DIR_CW : DIR_CCW, 20, 250, 0, true, false);
